...
export default async function scrapyDouban(
  link: string,
  ...
  if (doc.querySelector('#sec')) {
    throw new Error(
      'Failed to bypass Douban security page. Might need a proxy or manual login.',
    );
  }
...
function buildMovieItem(doc: Document) {
  const title =
    doc.querySelector('#content h1 [property="v:itemreviewed"]')?.textContent?.trim() ||
    '';
  ...
  const poster =
    img?.title === ImgDefaultTitle.Poster
      ? img?.src?.trim().replace(/\.webp$/, '.jpg')
      : '';
  ...
  const actorLinks = actorsPl[0]?.nextElementSibling?.querySelectorAll('span a');
  const actors = actorLinks
    ? [...actorLinks]
        .slice(0, 5)
        .map((i) => i.textContent)
        .join(' / ')
  ...
  const writerLinks = writersPl[0]?.nextElementSibling?.querySelectorAll('span a');
  const writers = writerLinks
    ? [...writerLinks]
        .slice(0, 5)
        .map((i) => i.textContent)
        .join(' / ')
  ...
  const genre = [...doc.querySelectorAll('#info [property="v:genre"]')]
    .map((i) => i.textContent || '')
    .filter((v) => v);
  const imdbInfo = [...doc.querySelectorAll(InfoSelector)].filter((i) =>
    i.textContent?.startsWith('IMDb'),
  );
  const imdbLink = imdbInfo.length
...
function buildMusicItem(doc: Document) {
  ...
  const img = doc.querySelector(ImgSelector) as HTMLImageElement;
  const cover =
    img?.title !== ImgDefaultTitle.Cover && img?.src.length <= 100
  ...
  if (release.length) {
    ...
    if (/\d{4}-\d/.test(text) && text) {
      ...
      releaseDate = dayjs(`${match![1]}-${match![2]}-${match![3]}`).format(
        'YYYY-MM-DD',
      );
    }
  }
...
function buildBookItem(doc: Document) {
  const title =
    doc.querySelector('#wrapper h1 [property="v:itemreviewed"]')?.textContent?.trim() ||
    '';
  const img = doc.querySelector(ImgSelector) as HTMLImageElement;
  const cover =
    img?.title !== ImgDefaultTitle.Cover && img?.src.length <= 100
  ...
  info.forEach((i) => {
    ...
    let nextText =
      i.nextSibling?.textContent?.trim() ||
      i.nextElementSibling?.textContent?.trim() ||
      '';

    if (text.startsWith('作者')) {
      writer =
        i.parentElement?.id === 'info'
      ...
      publisher =
        i.nextElementSibling?.tagName === 'BR' || !i.nextSibling?.textContent?.trim()
          ? nextText
...
function buildGameItem(doc: Document) {
  ...
  const gameInfo =
    doc.querySelector('#content .game-attr') ||
    (doc.querySelector('#content .thing-attr') as Element);
  const dts = [...gameInfo.querySelectorAll('dt')];
  const genre: string[] = [];
  let releaseDate = '';
  dts.forEach((dt) => {
    if (dt.textContent?.startsWith('类型')) {
      const strs = [...dt.nextElementSibling!.querySelectorAll('a')]
...
function buildDramaItem(doc: Document) {
  const title =
    doc.querySelector('#content .drama-info .meta h1')?.textContent?.trim() || '';
  const genre =
    doc
